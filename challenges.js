...
// given a 12-hour am/pm 

// function timeConversion to convert to military time 24hour format

// given a string '12:00:00PM'

// result is a string '19:05:45'

function timeConversion(time) {
  ampm = time.slice(-2)
  hours = parseInt(time.slice(0,2))
  restTime = time.slice(2)
  resultTime = ''
  if (ampm === 'PM') {
    newHours = (parseInt(hours) + 12).toString()
    resultTime = newHours.concat('', restTime)
    return resultTime.slice(0, -2)

  } else if (ampm === 'AM' && hours === 12) {
    resultTime = resultTime.concat('00', restTime)
    return resultTime.slice(0, -2)
  } else {
